...
**Figure 6. Native 2bRAD strain-level identification and abundance on the ATCC MSA-1002/1003 DNA mock.**
...
**Figure 12. In-silico shotgun strain-ID and abundance on MSA-1002/1003, vs StrainScan and inStrain.**
`figures/mock_shotgun.{png,pdf}` (generated by `scripts/plot_mock_strainid.py` + `scripts/plot_mock_toolcompare.py`;
Part II). All-enzyme combined 120-genome tree, `--min-abundance 0 --min-coverage 0.2`. **(A)** 20/20 strains
resolved with 0 false positives on both even and staggered mixes. **(B)** depth-based species abundance
recovering the even and staggered structures (staggered spans *E. coli* 25 % → *Bifidobacterium* 0.02 % by
mass). **(C)** strain-ID accuracy vs **StrainScan** and **inStrain** on the same even sample — all three
resolve 20/20. **(D)** wall time per sample: Strain2bScan ~66 s vs StrainScan ~35 min (emulated) vs inStrain
~52 min; inStrain did not finish the staggered sample (>6 h). Sources: `results/mock_strainid.tsv`,
`results/mock_tool_comparison.tsv`; doc: `docs/mock_strainid.md`.
