#!/usr/bin/env python3
"""Fig 7 (top row): saliva individual discrimination.
Build strain-level and species-level abundance matrices from Strain2bScan calls
on the 32 diurnal saliva 2bRAD samples; Bray-Curtis + PERMANOVA
...
for level in ("species", "strain"):
    ...
    tR2, tF, tp_ = permanova(D, tlabels)
    tacc = loo_1nn(D, tlabels)
    out.append(f"timepoint\t{level}\t{len(samples)}\t{M.shape[1]}\t{tR2:.4f}\t{tF:.3f}\t{tp_:.4f}\t{tacc:.3f}")
open(WORK / "saliva_permanova.tsv", "w").write("\n".join(out) + "\n")
...
with open(WORK / "saliva_perspecies_subject.tsv", "w") as w:
...
print(f"per-species: top = {perspec[0][4]} R2={perspec[0][0]:.3f}; "
      f"{sum(1 for x in perspec if x[1] < 0.05)}/{len(perspec)} species significant")
...
print(f"\nwrote {WORK / 'saliva_permanova.tsv'} + {WORK / 'saliva_perspecies_subject.tsv'}")
print(f"wrote {OUT_FIG / 'saliva_individual_discrimination.png/pdf'}")
